...
| Route guidance | At every route phase, assert exactly one critical objective is active and its beacon/arrow is available; physically complete each leg without activating Sonar. | A reviewer follows the primary hosted build without query-string shortcuts. |
...
| Presentation | Browser/UI regression verifies every live transition card keeps title, body, optional Combat Help, and Continue inside both 1280×720 and 1920×1080 viewports; dismissal returns to a usable world without HUD overlap. | Screenshots/GIF plus one complete manual hosted run show each handoff. |
...
| The QTE succeeded once and missed once, but both paths appeared to freeze on a blank battle screen. Pressing Enter eventually advanced without visible instruction. The dodge animation also read as a pole-spin rather than a dodge. | **Implemented:** treat this as a tutorial-handoff blocker. The resolved card keeps the labelled `Continue · Enter` acknowledgement and states the result in words: `Dodge succeeded — no damage taken.` or `Dodge missed — the attack landed normally.` The wording is not colour-only. The first compact card deliberately omits the detailed level-up table: stat gains still apply, while regular victories retain the full detail. Keep a stable dodge/fallback reaction if the imported animation does not read as a dodge. | `verify/tutorial_qte_handoff_layout.gd` drives the real moving QTE at production speed without injecting Enter. At both 1280×720 and 1920×1080, it asserts a positive battle stage, one visible acknowledgement, a fully visible outcome/instruction rect, a panel below the fixed header, no competing level-up table, and the explicit success/miss result phrase. The normal-entry browser smoke covers title → New Game → physical beam approach. Capture final hosted screenshots/GIF for both paths. | A first-time player can state what happened after the QTE, sees how to continue, and never interprets the screen as frozen. |
| The opening light beam began sideways from the default camera heading, so a browser run could leave a new player swimming without reaching the tutorial. | **Implemented:** place the rendered beam straight ahead on the default W axis. This does not lock or hard-code the world camera; it simply makes the first visible objective agree with the initial movement convention. | `verify/tutorial_forward_entry.gd` starts the title flow and drives production forward swimming until it requires a live tutorial battle. The hosted normal-entry browser check must also reach the same fight through the visible beam, not a query shortcut. | A new player sees the first destination ahead, holds W, and enters the tutorial without trial-and-error strafing. |
...
| A concise route card can still be clipped at browser height after a longer translation, optional Combat Help, or later transition adds copy. | **Implemented:** `RouteTransitionCard.visible_rects()` exposes only the rendered card/control bounds, and `verify/route_transition_layout.gd` drives the real World handoff boundaries for Shallows, both counter cards, lab approach, and Mermaid Freak preview. | Windowed 1280×720 and 1920×1080 checks require the panel, title, body, Help when promised, and Continue to be visible and non-overlapping. | Capture each hosted transition during the normal-entry route; a visible card is insufficient unless Continue returns to free movement. |
...
| A player reported an invisible barrier between party members in open water; it stretched far beyond the visible legacy highway rubble. | **Implemented:** the optional highway blockade retains its tall in-lane Shockwave gate, but its horizontal collision now ends with its visible 8 m lane instead of extending 60 m through open water. A query-only review entry starts beside the former plane and suppresses unrelated random encounters. | `verify/open_water_blockade.gd` starts at `(8, 2, 0)`, outside the lane at `z=10`, rejects a synthetic ordinary encounter, then drives the production diver right through `move_and_slide()` to `x>=22`. Before repair it stopped at `x=14.57`; after repair it passes. The existing 21-path route matrix also remains green. | Open `?open-water=1`, choose **Review Open-Water Crossing**, and hold **D** toward the party member. The crossing must remain clear; the visible highway rubble is deliberately off to the side. |
| Ignoring the route exposed an optional special encounter whose panels/controls were unintelligible because its ability had not been taught. | **Implemented:** `Optional Guardian Challenge` now names the guarded reward, says that it is off-route, gives the timed-challenge objective, and visibly lists each diver/ability control. It offers `Practice Controls`, `Enter Challenge`, and `Leave`; leaving preserves the site and never blocks the critical route. | `verify/optional_guardian_prompt.gd` enters a real guardian site and verifies all copy/actions, opens Practice Controls, then leaves to a live unpaused world without a battle or consumed guardian. | A player who deliberately leaves the beacon understands why the challenge appeared and how to decline or learn it. |
| The deeper-stat tutorial exists in Combat Help but was not discoverable during the route. | **Implemented:** route cards expose `Need a refresher? Combat Help` without consuming the active objective. Deep entry names Swordfish + Electric Touch + Evasion; the following card names Sea Urchin + Weaken + Defense, then returns control. | `verify/tutorial_ability_onboarding.gd` opens/closes Help from the actual handoff card and checks that Shallows remains active. `verify/progression_playthrough.gd` checks both just-in-time card payloads during the real World/Battle lifecycle. | A player can find the detailed lesson without knowing it exists beforehand and can explain each counter after the contextual prompt. |
| Shallows and Deep water looked equally bright; the route was a visually empty straight line of beacons. | **Implemented:** Deep/Lab phase changes densify fog, lower ambient light, shift to a deeper blue, and reveal non-colliding ruin silhouettes. The Shallows capstone now builds a broad, living **reef passage**—two coral/rock buttresses, a broken high arch, branching coral, and bioluminescent colour—around its named destination. The opening is deliberately 8 m wide, has no collision, and the route makes a broad lateral dogleg without depending on maze geometry. | `verify/route_phase_presentation.gd` asserts phase-applied values, landmarks, and non-collinear route turns. `verify/route_landmark.gd` checks that the capstone reports a reef passage and a real Diver crosses its center. Fixed-camera hosted captures still judge the art. | A reviewer can tell they entered Deep water before reading the label, and “secure the reef passage” now arrives at an identifiable place rather than another anonymous lamp. |
...
| Character naming was not immediately legible (`Maxilani`/`Maximilian` in the recording). | **Audit complete:** player-facing identities are centralized in `Cast.DISPLAY_NAMES` and the approved names remain Maxilani, Musashi, and Bucky. World HUD, Battle, ability onboarding, guardian selection, and Party Spells delegate to that source; no new canon name was invented. Historical notes and test fixtures retain older words only as non-player-facing context. | Existing `verify/pr54_merge_readiness.gd` creates a World and Battle and asserts all three visible names; the guardian prompt’s live copy derives the same Cast values. | A player can identify the active diver without having to infer who `Max` is. |
