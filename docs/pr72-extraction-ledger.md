...
| 2 — Combat Quick Read | pending PR | `27204ad` | `combat_quick_read`, `glassgoat_discord_followup`, `combat_feedback` — green | `verify/gates.sh` gameplay clean; stock browser sub-suite skipped because Playwright is absent | Manual local export: `?guardian=trench` → guardian → Attack → resolved choices → target → Back; no browser console errors at 1280×720 | Temporary export PCK SHA-256 `09cf66aab34c4f8f953ca8329f6c813ca20cb0db01a011755fda357ab95740f7`; immutable preview pending committed source | Ready for focused PR review |
