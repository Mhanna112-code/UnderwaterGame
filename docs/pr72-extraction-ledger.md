...
| 4 — Maze minimap | pending (`followup/maze-minimap-ux-v2`) | `fb45e8b` | `maze_minimap`, `maze`, `maze_traversal`, `maze_completion`, `maze_review_route` — green | Full `verify/gates.sh` is clean against this slice's isolated exact export, including every Chromium route (no browser skip). | Committed native closed/open M→H captures: `docs/evidence/maze-minimap-closed.png`, `docs/evidence/maze-minimap-open.png`; manual headful browser confirms `WindCorridor1` becomes `WindCorridor3` after H. | Slice export PCK SHA-256 recorded with its PR; a deploy from the finalized commit will be made before merge. | Ready for focused PR review |
