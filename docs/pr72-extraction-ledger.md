...
| 3 — Menus/spells/title | [PR #81](https://github.com/Mhanna112-code/UnderwaterGame/pull/81) | `90c1853` | `menus_spell_title` (Help scroll/replay, live title-route composition, Potion UI, legal spell learning), `persistence` (save/reload inventory/world rewards) — green | Final exact-head rerun pending after evidence commit | Committed browser captures: `docs/evidence/pr81-spell-review-title.png` and `docs/evidence/pr81-spell-review-tree.png`; query-gated route follows Title → Save → Update → Learn and the captured opaque tree has no visible world/HUD bleed. | Exact preview/PCK digest will be posted on PR #81 after the final exact-head export, avoiding self-referential export documentation. | In final verification |
